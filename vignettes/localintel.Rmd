---
title: "Getting Started with localintel"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Getting Started with localintel}
...
**localintel** (Local Intelligence) is an R package for fetching, processing,
and visualizing subnational (NUTS 0/1/2) data from Eurostat. It provides a
complete pipeline from raw Eurostat data to publication-ready maps and
...
The package provides pre-defined code lists for common health system datasets.
Use `fetch_eurostat_batch()` to download multiple datasets at once:
...
Each Eurostat dataset has a dedicated processing function that filters for the
correct indicators and reshapes the data:
...
Reference geometries and lookup tables are needed for cascading and mapping:
```{r reference}
...
The full cascade also computes derived indicators like Discharge Activity (DA)
and Relative Length of Stay (rLOS):
...
Create publication-ready maps that automatically select the best available
NUTS level for each country:
...
localintel works with standard Eurostat datasets. See `health_system_codes()`
and `causes_of_death_codes()` for the full list of pre-configured dataset
identifiers.
