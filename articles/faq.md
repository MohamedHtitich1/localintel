...
The package ships with curated registries for 14 domains. You can also
use
[`process_eurostat()`](https://mohamedhtitich1.github.io/localintel/reference/process_eurostat.md)
to process **any** Eurostat dataset not in the registry.

#### Q: My dataset isn’t in the registry. Can I still use localintel?

Absolutely. The core pipeline (fetch → process → cascade → visualize →
export) works with **any** Eurostat dataset. Use the generic workflow:
...
Priority order: NUTS 2 \> NUTS 1 \> NUTS 0. The `src_<variable>_level`
column tracks which level each value came from.
